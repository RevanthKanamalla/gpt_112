...
#to convert text to voice (google text to sppech)
#TITLE
st.title('Simple-GPT')
st.subheader('by Revanth')

# CREATING TABS
tab1, tab2 = st.tabs(["ASK AI","GENERATE IMAGE WITH AI"])

# OPENAI APIKEY
openai.api_key = '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'
...
with tab1:
    ...
    messages = [{"role": "system", "content": "Act like Your a men and Not ChatGPT."}]
    ...
    if st.button('Enter'):  #IF YOU CLICK THE BUTTION THEN RUN CODE BELOW
        ...
        tts=gTTS(text=reply, lang="en-US", slow=False)
        tts.save("reply.mp3")
